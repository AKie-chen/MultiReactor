...
完整数据与瓶颈分析见 [bench/README.md](../bench/README.md)（下方为结论摘要）。
...
- **线程数没有普适最优（2026-09 修正）**：8 核上动态路由 `-i 8 -w 1` 最优
  （`w=1` 比 `w=2/3` 快 7%~15%）；静态小文件正好相反，`w=2` 是 `w=1` 的 2.1 倍
  （50.6k → 106.7k）——缓存响应体的拷贝在 worker 线程上做，小文件路径每请求要
  ~29µs CPU，单 worker 扛不住。旧规则"IO ≈ 核数，worker 取 2~3"作废
- **闭环上限由压测端决定**：峰值 254k 时服务端只用 3.05 核，而同机的 wrk 用了
  4.34 核（16.5µs/请求，比服务端的 11.6µs 还贵），整机 92% 饱和；服务端独占 8 核
  按每请求成本外推 ≈692k req/s，本机测不出来（需把压测端移出 VM）
- **流水线峰值 472k 时整机只有 67%**（服务端 3.79 核 + 压测端 1.54 核，2.7 核空闲）：
  瓶颈不在算力，而在每 IO 线程的读-解析-提交-写回串行化与跨线程 hop
- c=2000/5000 的 503 即突发提交溢出在途上限的背压。在途上限是**全池单一计数器**
  （`ThreadPool::inFlight_`，判据 `inFlight_ >= maxQueueSize`），**与 worker 数无关**：
  `w=1` 与 `w=2` 容量相同（`-c3000` 达 9.9% 是并发档位决定的，不是 worker 数决定的）
...
### 为什么 Date 头做每秒缓存？
RFC 7231 §7.1.1.2 要求所有响应带 Date，且日期粒度是秒——同一秒内所有响应的 Date 头必须相同。全量响应序列化每请求取一次，数十万 req/s 下每请求一次 `gmtime_r + strftime`（两次系统调用 + locale 处理 ≈ 数百 ns）是纯浪费。`thread_local` 缓存上次的秒值与格式化结果：响应序列化只发生在 IO 线程（deliverResponse），各线程独立缓存，无锁无伪共享，秒边界首次访问才重算。
